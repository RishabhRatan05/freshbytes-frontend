  ...
  return (
    <div className=" text-white w-full h-full ">
      ...
      <div className="sm:grid grid-cols-4 bg-white">
        <aside className=" flex justify-center min-h-max bg-box col-span-1 p-1">
          <Categories />
        </aside>
        <div className="  col-span-3 pl-5 mt-5 ">
          {isLoading && <div className="text-black">Loading...</div>}
          {isSuccess &&
            data?.map((d) => {
              return (
                <div className="flex justify-start flex-col">
                  <div className="sm:text-2xl  text-xl bg-teal-400 w-fit px-2 my-2">
                    {d?.name}
                  </div>
                  <div className="flex flex-wrap flex-auto">
                    {d?.products?.map((product) => {
                      ...
                      console.log("qqqqq", product.name, qty)
                      return (
                        <ProductCard
                          _id={product._id}
                          key={product._id}
